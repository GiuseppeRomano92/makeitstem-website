...
Live at <https://makeitstem.com> (English), <https://makeitstem.com/it/> (Italian) and
<https://makeitstem.com/es/> (Spanish).
...
| Path | What it is |
|---|---|
| `src/pages/<lang>/*.html` (`en`, `it`, `es`) | Page content, one file per page per language. **Edit these.** |
...
| `*.html`, `it/*.html`, `es/*.html`, `sitemap.xml` | **Generated** — don't edit; they are overwritten on build |
...
- **Connect the course platform** — set `login_url` in `src/site.toml` (e.g. your
  LearnWorlds school's login page) and rebuild: "Log In" then goes straight there in every
  language, and the old `login.html` pages redirect to it.
...
- English is the default and keeps the original URLs (`/blog.html`); Italian and Spanish
  live under `/it/` and `/es/` (`/it/blog.html`, `/es/blog.html`). Section anchors (`#contact`, `#services`, …) are the same in
  every language, and the EN | IT | ES switcher keeps you on the same section.
...
- The Italian and Spanish privacy policy and terms are courtesy translations; they say that
